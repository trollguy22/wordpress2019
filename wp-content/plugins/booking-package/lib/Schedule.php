<?php
    ...
    class Schedule {
        ...
        public function __construct($prefix, $pluginName){
            ...
            #$this->setting = new setting($this->prefix, $this->pluginName);
        ...
        public function createUser($administrator = 0){
        	
        	$setting = new setting($this->prefix, $this->pluginName);
			...
			if(is_wp_error($user_id)){
				...
				$this->add_user($user_id, $_POST['user_email'], $activation, $hash);
				...
				if($activation == 1){
					...
					$this->sendMail(sanitize_text_field($_POST['user_email']), $subject, $body);
        ...
        public function updateUser($administrator = 0){
        ...
        public function get_users($authority, $offset, $number = null, $search = null){
        	...
        	foreach($users as $key => $user){
	        	...
	        	$sql = $wpdb->prepare(
	        		"SELECT `status`, `subscription_list` FROM `".$table_name."` WHERE `email` = %s;", 
        ...
        public function add_user($userId, $email, $activation, $hash = null){
        ...
        public function find_users($userId, $activation, $create = false){
			...
			if(is_null($row) && $create === true){
				...
				$this->add_user($userId, $user->user_email, $activation);
        ...
        public function update_subscription($subscription){
    		...
    		$creditCard = new creditCard($this->pluginName);
        ...
        public function setUserInformation($form){
    		...
    		global $wpdb;
    		$table_name = $wpdb->prefix."booking_package_users";
    		$setting = new setting($this->prefix, $this->plugin_name);
        	$memberSetting = array_merge($setting->getMemberSettingValues(), array('current_member_id' => 0, 'login' => 0));
    	...
    	public function getReservationData($month, $day, $year, $ical = false, $public = false){
			...
			if($ical === false){
				...
				$calendarList[intval($year.sprintf('%02d', $month))] = $arrayValue;
    		...
    		if(is_null($row)){
				...
				if($row['unixTime'] == $scheduleUnixTime){
					...
					if(isset($_POST['payToken'])){
						...
						$currency = get_option($this->prefix."currency", "usd");
						if(intval($payment_active) == 1 && !is_null($secret_key)){
							
							$creditCard = new creditCard($this->pluginName);
							$payResponse = $creditCard->pay($_POST['payType'], $public_key, $secret_key, $_POST['payToken'], $payment_live, $totalCost, $currency, $key, $name);
							if(isset($payResponse['error'])){
								
								#$wpdb->query('ROLLBACK');
								return array('status' => 'error', 'error' => $payResponse['error'], "totalCost" => $totalCost, "currency" => $currency, "totalCost" => $totalCost);
					...
					**/
					...
					if ($this->hasMultipleServices == 1) {
						...
						$privateResponse = $this->newInsertPrivateData($sendDate, $_POST['permission'], $status, $_POST['timeKey'], $scheduleUnixTime, $scheduleTitle, $scheduleCost, $services, $form, $currency, $_POST['payType'], $cardToken, $accountKey, $permalink, $preparation, $taxes, $applicantCount);
            ...
            if(is_null($row) === false){
				...
				**/
            	...
            	if(isset($_POST['update_booking_date']) || isset($_POST['update_booking_course'])){
    				...
    				$servicesDetails1 = $this->getSelectedServices($row['options'], "options", $applicantCount);
    		...
    		if (!is_null($services)) {
    			...
    			if (preg_match('/(\[service\])/', $emailBody, $matches)) {
    				...
    				if (is_array($services)) {
    					...
    					/**
    					$detailsList = array($service['name']);
    					if (intval($service['price']) > 0) {
    						
    						$detailsList = array($service['name']." ".$this->formatCost($service['price'], $currency));
    	...
    	public function sendMail($user_email, $subject, $body, $emailFormat = 'text'){
